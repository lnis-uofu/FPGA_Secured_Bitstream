...
def build_top(design="user_project_wrapper"):
    ...
    chip.add('input', 'verilog', 'rtl/user_project_wrapper.v')
    chip.add('input', 'verilog', 'rtl/fpga_top.bb.v')
    chip.add('input', 'verilog', 'rtl/pmu.bb.v')
    chip.add('input', 'verilog', 'rtl/aes.bb.v')
    chip.add('input', 'verilog', 'rtl/sha256.bb.v')
    ...
    for libname in ['pmu', 'fpga_top', 'aes', 'sha256']:
...
pdngen''')
    ...
    with open('build/macroplace_top.tcl', 'w') as mf:
        mf.write('''
# macros
place_cell -inst_name pmu_      -origin {1647.72 1797.92} -orient R0 -status FIRM
place_cell -inst_name fpga_top_ -origin {1647.72  816} -orient R0 -status FIRM
place_cell -inst_name sha256_   -origin {674.36  995.52} -orient R0 -status FIRM
place_cell -inst_name aes128_   -origin {477.48  1797.92} -orient R0 -status FIRM
''')
    chip.set('pdk', pdk, 'aprtech', 'openroad', stackup, libtype, 'macroplace', 'build/macroplace_top.tcl')

    chip.set('output', 'spef', 'user_project_wrapper.spef')
