...
def build_pmu(design="sha256"):
    chip = Chip(design)
    chip.load_target('skywater130_demo')

    # Load all source
    #
    chip.add('input', 'verilog', './rtl/sha256.v')
    chip.add('input', 'verilog', '../../../pmu/rtl/sha/sha256_core.v')
    chip.add('input', 'verilog', '../../../pmu/rtl/sha/sha256_k_constants.v')
    chip.add('input', 'verilog', '../../../pmu/rtl/sha/sha256_w_mem.v')


    # Design constraints
    chip.set('input', 'sdc', 'sdc/sha.sdc')

    chip.set('tool', 'openroad', 'var', 'place', '0', 'place_density', ['0.38'])
