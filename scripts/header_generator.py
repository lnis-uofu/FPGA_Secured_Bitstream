...
def header_generator(file_in, size, starting_address, instruction, file_out):
    f = open(file_in, "r")
    ...
    header = ""
    bin_size = bin(size)[2:].zfill(32)
    mem_addr = bin(starting_address)[2:].zfill(28)
    header = str(bin_size) + str(mem_addr) + instruction
...
if __name__ == "__main__":
    ...
    ap.add_argument("starting_address", type=int , help="starting address when loading memory")
    ...
    args = ap.parse_args()
    header_generator(args.file_in, args.bitstream_size, args.starting_address, args.instruction, args.file_out)
